...
def generate_control_rom():
    ...
    for i in range(len(sequence)):
        if serial[sequence[i]] in R_TYPE:  # Register Only
            print(generate_control_rom_row(REG_DST_0_3, ALU_SRC_REG, MEM_TO_REG_ALU_RESULT,
                  '1', '0', '0', '0', '0', '0', ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in S_TYPE:  # Shift
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_ALU_RESULT,
                  '1', '0', '0', '0', '0', '0', ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[:4]:  # immediate
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_ALU_RESULT,
                  '1', '0', '0', '0', '0', '0', ALU_OP[serial[sequence[i]][:-1]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[4:6]:  # Branch
            is_beq = ('1', '0') if serial[sequence[i]] == 'beq' else ('0', '1')
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_REG, MEM_TO_REG_ALU_RESULT,
                  '0', '0', '0', is_beq[0], is_beq[1], '0', ALU_OP['sub']), end=' ')
        elif serial[sequence[i]] in I_TYPE[6:8]:  # Memory
            if serial[sequence[i]] == 'sw':
                print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST,
                      MEM_TO_REG_MEM_RESULT, '0', '0', '1', '0', '0', '0', ALU_OP['add']), end=' ')
            else:
                print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST,
                      MEM_TO_REG_MEM_RESULT, '1', '1', '0', '0', '0', '0', ALU_OP['add']), end=' ')
        elif serial[sequence[i]] in J_TYPE:  # Jump
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_REG,
                  MEM_TO_REG_ALU_RESULT, '0', '0', '0', '0', '0', '1', ALU_OP['add']), end=' ')
    print()
    ...
    # lw $t0, 1($t1) : Opcode SrcReg1($t1) DstReg($t0) signed-int       | *($t1+1) = $t0
        ...
        if opcode in I_TYPE[:6]:
            ...
            if opcode in I_TYPE[:4]:  # Immediate Operations
                ...
                immediate = labels[args[2]] - \
                    (line_no + 1)  # PC auto-increments
...
if __name__ == '__main__':
    ...
    with open('input.asm', 'r') as file:
        lines = [line.split(r'//')[0].strip()
                 for line in file]                 # Trim Whitespaces
        # Without Empty Lines
        lines = list(filter(lambda line: line != '', lines))

    # Label Location Generated
    ...
    with open('output.bin', 'w') as bin_file:
        for i in range(len(lines)):
            bin_file.write(' '.join(
                [bin_lines[i][j:j+4] for j in range(0, len(bin_lines[i]), 4)]) + ' : ' + lines[i] + '\n')
