...
void BasicToCode(vector<vector<string>> &ccode)
{
    ...
    out << "int count = " << ccode.size() << ";" << endl;
    out << "uint16_t INSTRUCTIONS[" << ccode.size() << "] = {";
    cout << "\nCODE:" << endl;
    for (int i = 0; i < ccode.size(); i++)
    {
        ...
        case 'R':
        {
            out << "0x" << opcode << ccode[i][2] << ccode[i][3] << ccode[i][1];
        ...
        case 'S':
        {
            out << "0x" << opcode << ccode[i][2] << ccode[i][1] << ccode[i][3];
        ...
        case 'J':
        {
            out << "0x" << opcode << setfill('0') << setw(2) << ccode[i][1] << '0';
        ...
        case 'I':
        {
            ...
            {
                out << "0x" << opcode << ccode[i][3] << ccode[i][1] << ccode[i][2];
                cout << "0x" << opcode << ccode[i][3] << ccode[i][1] << ccode[i][2] << endl;
            }
            else if (ccode[i][0] == "beq" || ccode[i][0] == "bneq")
            {
                out << "0x" << opcode << ccode[i][1] << ccode[i][2] << ccode[i][3];
...
int main()
{
    ...
    cout << "SOURCE:" << endl;
    // ccode.push_back(tokenize("addi $sp, $zero, 15"));
    while (in >> start)
    {
        getline(in, line);
        string tmp;
        ...
        else if (start == "push")
        {
            ...
            // ccode.push_back(tokenize("sw " + tokens[0] + ", 0($sp)"));
            // ccode.push_back(tokenize("subi $sp, $sp, 1"));
            ccode.push_back(tokens);
        }
        else if (start == "pop")
        {
            vector<string> tokens = tokenize(line);
            // ccode.push_back(tokenize("addi $sp, $sp, 1"));
            // ccode.push_back(tokenize("lw " + tokens[0] + ", 0($sp)"));
            ccode.push_back(tokens);
        }
        else
        {
